...
class CPU:
        ...
        for wave_function, description in collection.get_wave_functions():
            ...
            for j in range(cpu_iter):
                if isinstance(wave_function, INoiseWrapper):
                    opt_value, par = CPU.get_custom_optimized_state(n_w=copy.deepcopy(wave_function), max_cpu_iter=cpu_iter, max_qpu_iter=qpu_iter)  # Mixed
    ...
    @staticmethod
    def get_custom_optimized_state(n_w: INoiseWrapper, max_cpu_iter: int, max_qpu_iter: int) -> (float, List[float]):
        """
        ...
        :param max_cpu_iter: Maximum iteration steps for (classic) scipy optimize
        :return: Expectation value of optimized circuit, optimized circuit parameters
        """
        ...
        expectation_function, _cost = n_w.observable_measurement()
        ...
        def evaluate_circuit(p: IParameter) -> float:
            # exp_value = QPU.get_realistic_noisy_expectation_value(n_w=n_w, c=evaluation_circuit, p=p, max_iter=max_qpu_iter)
            # Resolve circuit
            circuit_to_run = QPU.get_resolved_circuit(c=evaluation_circuit, p=p)
            return expectation_function(circuit_to_run, n_w._noise_channel, max_qpu_iter)

        def optimize_func(x: np.ndarray) -> float:
            update_variational_parameters(p=operator_params, v=x)
            result = evaluate_circuit(p=operator_params)
            return result

        initial_values = np.fromiter(operator_params.dict.values(), dtype=float)
        method = 'COBYLA'  # 'Nelder-Mead'
        options = {'maxiter': max_cpu_iter}
        optimize_result = optimize.minimize(fun=optimize_func, x0=initial_values, method=method, options=options, tol=1e-6)
    ...
    def get_mitigated_optimized_state(w: IWaveFunction, n: INoiseModel, max_optimization_iter: int, max_mitigation_count: int) -> float:
        ...
        noisy_ansatz = INoiseWrapper(w_class=w, noise_model=n)
        ...
        if clean_optimization:
            ...
            circuit_parameters.update(v=result.optimal_parameters)
        else:
            # Prepare noisy ansatz optimization
            values, params = CPU.get_custom_optimized_state(n_w=noisy_ansatz, max_cpu_iter=max_optimization_iter, max_qpu_iter=max_mitigation_count)
